<?php

namespace App\Http\Requests\CategoryCourse;

use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Http\JsonResponse;

class CategoryCourseStoreRequest extends FormRequest
{
    /**
     * Determine if the user is authorized to make this request.
     *
     * @return bool
     */
    public function authorize()
    {
        return true;
    }

    /**
     * Get the validation rules that apply to the request.
     *
     * @return array<string, mixed>
     */
    public function rules()
    {
        return [
            'id' =>  'integer|exists:course_categories,id',
            'title' =>  'string|nullable|max:255',
            'tag_id' =>  'integer|exists:filter_category_tag,id',
            'parent_ids' =>  'array',
            'parent_ids.*' =>  'exists:course_categories,id',
            'description' =>  'string|nullable|max:255',
            'seo_title' =>  'string|nullable|max:255',
            'seo_description' =>  'string|nullable|max:255',
            'seo_keywords' =>  'string|nullable|max:255',
        ];
    }

    public function messages()
    {
        return [
            'title.required' => 'Название категории должно быть заполненно',
            'tag_id.required' => 'Тэг фильтрации должен быть установлен',
            'tag_id.exists' => 'Нет тэга фильтрации с таким id',
            'parent_ids.*.exists' => 'Нет родительской категории с id::input',
            'id.exists' => 'Нет категории с таким id',
        ];
    }

    protected function prepareForValidation()
    {
        if($this->title){
            $this->merge(['slug' => \Str::slug($this->title)]);
        }
        if($this->status === null){
            $this->offsetUnset('status');
        }
    }
    
    protected function failedValidation(\Illuminate\Contracts\Validation\Validator $validator)
    {
        $response = new JsonResponse([
            'code' => 404,
            'message' => $validator->errors()->first()
        ], 404);
        throw new \Illuminate\Validation\ValidationException($validator, $response);
    }
}
