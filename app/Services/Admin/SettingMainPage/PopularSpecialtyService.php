...
{   
    ...
    {
        // return PopularSpecialty::find(1);
        $specialtiesChecked = CategoryCourse::find(PopularSpecialty::find(1)->append('specialties')->specialties);

        return $specialtiesChecked->map(function ($spec) {
            return collect($spec->toArray())
                ->only(['id', 'title'])
                ->all();
        });
